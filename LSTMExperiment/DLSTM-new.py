...
def get_train_data(batch_size=60, time_step=20, train_begin=0, train_end=80000):
    batch_index = []
...
def get_test_data(time_step=20, test_begin = 80000):
    ...
    for i in range(len(normalized_test_data)-time_step-1):
        ...
        y = normalized_test_data[i+1:i+time_step+1]
...
class get_stock_data():
    ...
    #         orgin_data_path = os.path.join(basic_path, "stats.csv")
    ...
    #     # total_data["gate"] = np.round(
    #     #     (100 * (g_stock_num.shift(-2)["open_hfq"] / g_stock_num.shift(-1)["open_hfq"] - 1)), 2)
    #     # 重新调整列的顺序，为接下来处理成输入、输出形式做准备
    #     # total_data = total_data[["CPU"]]
    #
    ...
    def get_train_test_data_softmax(self, batch_size, file_path, time_step=60):
        ...
        return np.array(data_temp["0"].tolist()).reshape(batch_size, time_step * 7), np.array(
            data_temp["1"].tolist()).reshape(batch_size, 3)
...
class Deeplearing():
    ...
    def stock_lstm(self, ):
        ...
        input_rnn = tf.reshape(input_rnn, [-1, origin_data_row, cell_num])  # [-1,60,256]

        # 定义一个带着“开关”的LSTM单层，一般管它叫细胞
        def lstm_cell():
            cell = rnn.LSTMCell(cell_num, reuse=tf.get_variable_scope().reuse)  # 默认：forget_bias=1.0  表示不忘记数据
        ...
        accuracy = tf.reduce_max(tf.abs(tf.subtract(y_pre, input_y)))  # 1.subtract：减法操作   2.reduce_max计算张量的各个维度上元素的最大值

        # tf要求必须如此定义一个init变量，用以在初始化运行（也就是没有保存模型）时加载各个变量
        init = tf.global_variables_initializer()

        # 用以保存参数的函数（跑完下次再跑，就可以直接读取上次跑的结果而不必重头开始）
        # saver = tf.train.Saver()   # 原始
        saver = tf.train.Saver(tf.global_variables())  # FL
        # 获取数据（这是我们自己定义的类）
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(1000):
                try:
                    # 读取训练集数据
                    batch_index, train_x, train_y = get_train_data()  # list
                    ...
                    saver.save(sess, 'model_save\\modle.ckpt')
                    print("保存模型\n")
                    break
                ...
                if (i + 1) % 20 == 0:
                    ...
                    # 输出
                    print("step: {0}, training_accuracy: {1}".format(i + 1, train_accuracy))
                    # saver.save(sess, model_save_path)   # 原始
                    saver.save(sess, 'model_save\\modle.ckpt')
                    print("保存模型\n")
                    ############################################
